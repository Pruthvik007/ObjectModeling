...
public class UserService implements IUserService {
    ...
    Comparator<User> orderUsersInAsc=new Comparator<User>() {
        public int compare(User u1, User u2){
            return u1.getScore()-u2.getScore();
        }
     };
     if(ScoreOrder.ASC.equals(scoreOrder)){
        Collections.sort(users,orderUsersInAsc);
     } else {
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName) throws ContestNotFoundException, UserNotFoundException, InvalidOperationException {
     ...
     if(!user.get().checkIfContestExists(contest.get()) || contest.get().getContestStatus().equals(ContestStatus.ENDED) || contest.get().getContestStatus().equals(ContestStatus.IN_PROGRESS)
        || user.get().getId().equals(contest.get().getCreator().getId())) {
